...
function display_cff($atts, $content = null) {
    ...
    foreach ( $page_ids as $page_id ) {
        ...
        if ($cff_events_only && $cff_events_source == 'eventspage'){
            ...
            if($cff_is_group) $cff_events_json_url = 'https://graph.facebook.com/v2.5/' . $page_id . '/events?fields=name,id,description,start_time,end_time,timezone,ticket_uri,place,cover&limit=200&since='.date('Y-m-d').'&access_token=' . $access_token;
        ...
        if (!$cff_events_only || ($cff_events_only && $cff_events_source == 'timeline') ){
            ...
            if($cff_albums_only && $cff_albums_source == 'photospage'){
                ...
                if($cff_is_group) $cff_posts_json_url = 'https://graph.facebook.com/' . $page_id . '/albums?fields=created,name,count,cover_pid,link,modified,id&access_token=' . $access_token . '&limit=' . $cff_post_limit. '&locale=' . $cff_locale . $cff_date_range;
            }
                    ...
                    if($cff_offset_show_post){
                            ...
                            if ($cff_show_event_title || $cff_show_event_details) {
                                ...
                                if ($cff_post_type == 'event') {
                                    ...
                                    if( isset($event_object->cover) ){
                                        $cff_timeline_event_photo = $event_object->cover->source;
                                        $cff_event_has_cover_photo = true;
                                    } else {
                                        $cff_timeline_event_photo = 'https://graph.facebook.com/'.$eventID.'/picture?width=200&amp;height=200';
                                    }

                                    if($cff_show_media){
                                        $cff_event .= '<a title="'.$cff_facebook_link_text.'" class="cff-event-thumb';
                                        if($cff_event_has_cover_photo) $cff_event .= ' cff-has-cover';
                                        $cff_event .= '" href="'.$link.'" '.$target.$cff_nofollow.'><img src="'.$cff_timeline_event_photo.'" alt="'.$cff_facebook_link_text.'" /></a>';
                                    }
                        ...
                        if($cff_albums_only && $cff_albums_source == 'photospage'){
                            ...
                            if( $cff_show_post ){
                                ...
                                } else if( isset($news->cover_pid) ){ //Groups so need to use token in the request:
                                    $thumb = 'https://graph.facebook.com/' . $news->cover_pid . '/picture?access_token='.$access_token;
                                } else {
                                    $thumb = '';
                                ...
                                if( $cff_cover_photos_available ) $cff_post_item .= '<a href="' . $cff_album_link . '" class="cff-album-cover" '.$target.$cff_nofollow.'><img src="'.$thumb.'" alt="' . $cff_album_name . '" /></a>';
