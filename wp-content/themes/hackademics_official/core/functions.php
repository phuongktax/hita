...
add_filter( 'manage_edit-recruitment_columns', 'my_edit_recruitment_columns' ) ;

function my_edit_recruitment_columns( $columns ) {
	$new = array('companyname' => __( 'Tên Công Ty' ),
		'deadline' => __( 'Hạn Nộp Hồ Sơn' ),);
	$add = array_slice($columns,0,2,true)+$new+array_slice($columns,2,count($columns),true);	

	return $add;
}

add_action( 'manage_recruitment_posts_custom_column', 'my_manage_recruitment_columns', 10, 2 );
...
function wpse45436_posts_filter( $query ){
    ...
    if ( 'course' == $type && is_admin() && $pagenow=='edit.php' && isset($_GET['ADMIN_FILTER_FIELD_VALUE']) && $_GET['ADMIN_FILTER_FIELD_VALUE'] != '') {
...
$query = new WP_Query( $args );
       ...
       //$query->query_vars['meta_key'] = 'classname';
...
add_filter( 'parse_query', 'jobname_filter' );
function jobname_filter( $query ){
    ...
    if ( 'recruitment' == $type && is_admin() && $pagenow=='edit.php' && isset($_GET['ADMIN_FILTER_FIELD_VALUE']) && $_GET['ADMIN_FILTER_FIELD_VALUE'] != '') {
        $query->query_vars['meta_key'] = 'jobname';
        $query->query_vars['meta_value'] = $_GET['ADMIN_FILTER_FIELD_VALUE'];
    }
}
