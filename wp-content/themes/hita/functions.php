...
/**
  ...
  **/
  if ( ! function_exists( 'hita_theme_setup' ) ) {
        /*
         * Nếu chưa có hàm hita_theme_setup() thì sẽ tạo mới hàm đó
         */
        function hita_theme_setup() {
                /*
                 * Thiết lập theme có thể dịch được
                 */
                $language_folder = THEME_URL . '/languages';
                load_theme_textdomain( 'hita', $language_folder );
                 ...
                 * Tạo menu cho theme
                 */
                 register_nav_menu ( 'primary-menu', __('Primary Menu', 'hita') );
                /*
                 * Tạo sidebar cho theme
                 */
                 $sidebar = array(
                    'name' => __('Main Sidebar', 'hita'),
                    'id' => 'main-sidebar',
                    'description' => 'Main sidebar for hita theme',
                    'class' => 'main-sidebar',
        ...
        add_action ( 'init', 'hita_theme_setup' );
  }
...
* hita_menu( $slug )
**/
if ( ! function_exists( 'hita_menu' ) ) {
  function hita_menu( $slug ) {
    $menu = array(
      'theme_location' => $slug,
      'container' => 'nav',
...
* hita_pagination()
**/
if ( ! function_exists( 'hita_pagination' ) ) {
  function hita_pagination() {
    /*
  ...
  <nav class="pagination" role="navigation">
    <?php if ( get_next_post_link() ) : ?>
      <div class="prev"><?php next_posts_link( __('Older Posts', 'hita') ); ?></div>
    <?php endif; ?>
    <?php if ( get_previous_post_link() ) : ?>
      <div class="next"><?php previous_posts_link( __('Newer Posts', 'hita') ); ?></div>
    <?php endif; ?>
  </nav><?php
  }
}
...
* hita_thumbnail( $size )
**/
if ( ! function_exists( 'hita_thumbnail' ) ) {
  function hita_thumbnail( $size ) {
...
* hita_entry_header()
**/
if ( ! function_exists( 'hita_entry_header' ) ) {
  function hita_entry_header() {
    if ( is_single() ) : ?>
...
* hita_entry_meta()
**/
if( ! function_exists( 'hita_entry_meta' ) ) {
  function hita_entry_meta() {
    if ( ! is_page() ) :
      echo '<div class="entry-meta">';
        // Hiển thị tên tác giả, tên category và ngày tháng đăng bài
        printf( __('<span class="author">Posted by %1$s</span>', 'hita'),
          get_the_author() );
        printf( __('<span class="date-published"> at %1$s</span>', 'hita'),
          get_the_date() );
        printf( __('<span class="category"> in %1$s</span>', 'hita'),
          get_the_category_list( ', ' ) );
        // Hiển thị số đếm lượt bình luận
        if ( comments_open() ) :
          echo ' <span class="meta-reply">';
            comments_popup_link(
              __('Leave a comment', 'hita'),
              __('One comment', 'hita'),
              __('% comments', 'hita'),
              __('Read all comments', 'hita')
             );
          echo '</span>';
        endif;
      echo '</div>';
 ...
 */
function hita_readmore() {
  return '...<a class="read-more" href="'. get_permalink( get_the_ID() ) . '">' . __('Read More', 'hita') . '</a>';
}
add_filter( 'excerpt_more', 'hita_readmore' );
  /**
  ...
  * hita_entry_content()
  **/
  if ( ! function_exists( 'hita_entry_content' ) ) {
    function hita_entry_content() {
      if ( ! is_single() ) :
        the_excerpt();
      else :
        the_content();
         ...
         */
        $link_pages = array(
          'before' => __('<p>Page:', 'hita'),
          'after' => '</p>',
          'nextpagelink'     => __( 'Next page', 'hita' ),
          'previouspagelink' => __( 'Previous page', 'hita' )
        );
...
* hita_entry_tag()
**/
if ( ! function_exists( 'hita_entry_tag' ) ) {
  function hita_entry_tag() {
    if ( has_tag() ) :
      echo '<div class="entry-tag">';
      printf( __('Tagged in %1$s', 'hita'), get_the_tag_list( '', ', ' ) );
      echo '</div>';
    endif;
  }
}
